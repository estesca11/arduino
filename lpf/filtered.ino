...
const int threshold = 1.8;
const int ledPin[6] = {8, 9, 10, 11, 12, 13};
int Sensor1 = 0;
#define sensorNum 1
double volts[sensorNum] = {0,};
unsigned int sample[sensorNum] = {0,};
double volts_value[sensorNum] = {0,};
...
void loop()
{
    double af_value[sensorNum] = {0,};
    unsigned long now = millis();
    unsigned int peakToPeak[sensorNum] = {0,};
    unsigned int sigMin[sensorNum]={0,};
    for(int i = 0; i < sensorNum; i++){
        sigMin[i] = 1023;
    }
    unsigned int sigMax[sensorNum] = {0,};
